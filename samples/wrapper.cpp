...
int lc_mp4_muxer_write_frame(void *muxer, int is_video, uint8_t *frame, int size, int64_t pts)
{
    ...
    LC_MP4_MUXER_INFO_t* mux = (LC_MP4_MUXER_INFO_t*)muxer;
    if (is_video) {
        ret = on_data_video(mux, frame, size, pts);
    } else {
        ret = on_data_audio(mux, frame, size, pts);
    }
...
int on_data_audio(LC_MP4_MUXER_INFO_t *mux, uint8_t *frame, int size, int64_t pts)
{
    int ret = -1;
    ...
    if (mux->info.audio_track_id == MP4_INVALID_TRACK_ID) {
        if (mux->info.audio_codec == LC_MP4_CODEC_PCMA || mux->info.audio_codec == LC_MP4_CODEC_PCMU) {
            ...
            // MP4SetTrackSamplesPerChunk(mux->hFile, mux->info.audio_track_id, 1);
    ...
    if (MP4WriteSample(mux->hFile, mux->info.audio_track_id, frame, size, mux->info.audio_duration)) {
        ret = 0;
    }
...
int on_data_video(LC_MP4_MUXER_INFO_t *mux, uint8_t *frame, int size, int64_t pts)
{
    // parse nalu type
    int ret = -1;
    int offset = 0, len = 0;
    ...
    if (mux->last_video_ts != 0 && pts > mux->last_video_ts) {
        ...
        if  (du < duration * 4) {
            ...
            // printf("ERR pts = %" PRId64 ", mux->last_video_ts = %" PRId64 "\n", pts, mux->last_video_ts);
        }
    }
    ...
    while ((len = get_one_nalu_from_buf(frame, size, offset, &nalu)) > 0 ) {
        ...
        if (mux->info.video_codec == LC_MP4_CODEC_H264) {
            ret = handle_video_h264(mux, nalu.nal_unit , nalu.nal_unit_size, duration);
        } else if (mux->info.video_codec == LC_MP4_CODEC_H265){
            ret = handle_video_h265(mux, nalu.nal_unit , nalu.nal_unit_size, duration);
        } else {
        ...
        case LC_MP4_AVC_NAL_UNIT_TYPE_SPS:
            {
                ...
                // printf("mux->info.w = %u, mux->info.h = %u\n", mux->info.w, mux->info.h);
                ...
                if (mux->info.video_track_id == MP4_INVALID_TRACK_ID) {
                    ...
                    // Return and need to open a new file
                    if (mux->info.w != (int)width || mux->info.h != (int)height) {
                        return -2;
                    }
                }
            }
        ...
        case LC_MP4_HEVC_NAL_UNIT_TYPE_SPS_NUT:
            {
                ...
                unsigned char *output_data = (unsigned char *)malloc(nal_unit_size);
                if (output_data == NULL) {
                    return -1;
                }

                int output_len = remove_emulation_prevention_three_byte(nal_unit, nal_unit_size, output_data);

                LC_MP4_HevcSequenceParameterSet sps;
                sps.Parse(output_data, output_len);
                uint32_t width, height;
                sps.GetWidthHeight(width, height);

                free(output_data);

                MP4Duration track_duration = MP4GetTrackDuration(mux->hFile, mux->info.video_track_id);
                if (width != 0 && height != 0) {
                    if (mux->info.w != (int)width || mux->info.h != (int)height) {
                        if (track_duration == 0) {
                            MP4SetTrackFloatProperty(mux->hFile, mux->info.video_track_id, "tkhd.width", width);
                            MP4SetTrackFloatProperty(mux->hFile, mux->info.video_track_id, "tkhd.height", height);
                            MP4SetTrackIntegerProperty(mux->hFile, mux->info.video_track_id, "mdia.minf.stbl.stsd.hev1.width", width);
                            MP4SetTrackIntegerProperty(mux->hFile, mux->info.video_track_id, "mdia.minf.stbl.stsd.hev1.height", height);
                            mux->info.w = width;
                            mux->info.h = height;
                        } else {
                            return -2;
