...
+---------------------------------------------------------------------*/
static int
SignedGolomb(uint32_t code_num)
{
    if (code_num % 2) {
        return (code_num+1)/2;
    } else {
...
// Removes emulation_prevention_three_byte (0x000003) from the input buffer.
int remove_emulation_prevention_three_byte(const uint8_t *input, int input_len, uint8_t *output)
{
    // Initialize the output length to 0
    int output_len = 0;
    // Initialize the input index to 0
    int i = 0;

    // Iterate through the input buffer
    while (i < input_len) {
        // Check if the current three bytes are 0x000003
        if (i + 2 < input_len && input[i] == 0x00 && input[i+1] == 0x00 && input[i+2] == 0x03) {
            // Replace 0x000003 with 0x0000 in the output buffer
            output[output_len++] = input[i++];
            output[output_len++] = input[i++];
            // Skip the 0x03 byte
            i++; 
        } else {
            // Copy the current byte to the output buffer
            output[output_len++] = input[i++];
        }
    }
...
void LC_MP4_AvcSequenceParameterSet::GetWidthHeight(uint32_t& width, uint32_t& height)
{
...
int LC_MP4_AvcSequenceParameterSet::Parse(const uint8_t* data, uint32_t data_size)
{
    ...
    if (seq_parameter_set_id > LC_MP4_AVC_SPS_MAX_ID) {
        return -10;
    }
    ...
    if (profile_idc  ==  100  ||
        ...
        qpprime_y_zero_transform_bypass_flag = bits.ReadBit();
        seq_scaling_matrix_present_flag = bits.ReadBit();
    ...
    log2_max_frame_num_minus4 = ReadGolomb(bits);
    pic_order_cnt_type = ReadGolomb(bits);
    ...
    if (pic_order_cnt_type == 0) {
        ...
        delta_pic_order_always_zero_flags = bits.ReadBit();
        offset_for_non_ref_pic = SignedGolomb(ReadGolomb(bits));
        offset_for_top_to_bottom_field = SignedGolomb(ReadGolomb(bits));
        num_ref_frames_in_pic_order_cnt_cycle = ReadGolomb(bits);
        if (num_ref_frames_in_pic_order_cnt_cycle > LC_MP4_AVC_SPS_MAX_NUM_REF_FRAMES_IN_PIC_ORDER_CNT_CYCLE) {
            return -10;
        }
        for (unsigned int i=0; i<num_ref_frames_in_pic_order_cnt_cycle; i++) {
            offset_for_ref_frame[i] = SignedGolomb(ReadGolomb(bits));
        }
    }
    num_ref_frames                       = ReadGolomb(bits);
    gaps_in_frame_num_value_allowed_flag = bits.ReadBit();
    pic_width_in_mbs_minus1              = ReadGolomb(bits);
    pic_height_in_map_units_minus1       = ReadGolomb(bits);
    frame_mbs_only_flag                  = bits.ReadBit();
    if (!frame_mbs_only_flag) {
        mb_adaptive_frame_field_flag = bits.ReadBit();
    }
    direct_8x8_inference_flag = bits.ReadBit();
    frame_cropping_flag       = bits.ReadBit();
    if (frame_cropping_flag) {
        frame_crop_left_offset   = ReadGolomb(bits);
        frame_crop_right_offset  = ReadGolomb(bits);
        frame_crop_top_offset    = ReadGolomb(bits);
        frame_crop_bottom_offset = ReadGolomb(bits);
    }
...
/*----------------------------------------------------------------------
|   LC_MP4_HevcSequenceParameterSet::GetWidthHeight
+---------------------------------------------------------------------*/
void LC_MP4_HevcSequenceParameterSet::GetWidthHeight(uint32_t& width, uint32_t& height)
{
    unsigned int crop_x = 0, crop_y = 0, sub_width_c = 0, sub_height_c = 0;
    ...
    if (conformance_window_flag != 0) {
        ...
        // Formula D-28, D-29
        crop_x = sub_width_c * (conf_win_right_offset + conf_win_left_offset);
        crop_y = sub_height_c * (conf_win_bottom_offset + conf_win_top_offset);
    }

    width = pic_width_in_luma_samples - crop_x;
    height = pic_height_in_luma_samples - crop_y;
}
