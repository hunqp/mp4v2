...
int maz_split(char *iname)
{
    ...
    */
    ...
    while(1)
    {
        ...
        dlog(DBUG, "[%04d] type:%d len:%d", scnt, hd.type, hd.len);
            ...
            case 32:            // VPS
                if (flag)
                {
                    vid = MP4AddH265VideoTrack(hdl,
                                               ...
                                               width,                   // width
                                               height,                  // height
                                               1,                       // sps[1] AVCProfileIndication
                                               0,                       // sps[2] profile_compat
                                               150,                     // sps[3] AVCLevelIndication
                ...
                dmsg(DIGN, "[%04d] VPS(%d)\n", vcnt, hd.len);
                MP4AddH265VideoParameterSet(hdl, vid, nalu, hd.len - 4);
                break;
            case 33:            // SPS
                dmsg(DIGN, "[%04d] SPS(%d)\n", vcnt, hd.len);
                MP4AddH265SequenceParameterSet(hdl, vid, nalu, hd.len - 4);
                break;
            case 34:            // PPS
                dmsg(DIGN, "[%04d] PPS(%d)\n", vcnt, hd.len);
                MP4AddH265PictureParameterSet(hdl, vid, nalu, hd.len - 4);
                break;
            default:
                break;
            }
